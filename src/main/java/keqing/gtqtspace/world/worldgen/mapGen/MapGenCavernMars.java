...
public class MapGenCavernMars extends MapGenBaseMeta {
    IBlockState testBlock = GTQTSMetaBlocks.GTQTS_STONE_BLOCKS.get(GTQTSStoneVariantBlock.StoneVariant.SMOOTH).getState(GTQTSStoneVariantBlock.StoneType.MARS);
                                ...
                                for (int var50 = caveMaxY - 1; var50 >= caveMinY; --var50) {
                                    ...
                                    if (var59 * var59 + var51 * var51 + var46 * var46 < 1.0D) {
                                        if (var51 > -0.7D) {
                                            ...
                                            IBlockState blockAbove = primer.getBlockState(var42, var50 + 1, var45);
                                            IBlockState blockBelow = primer.getBlockState(var42, var50 - 1, var45);

                                            if (Blocks.AIR == block) {
                                                if (blockAbove == testBlock && this.rand.nextInt(200) == 0) {
                                                    int modifier = 0;

                                                    while (Blocks.AIR.getDefaultState() == block && var50 + modifier > 0) {
                                                        primer.setBlockState(var42, var50 + modifier, var45, testBlock);
                                                        modifier--;
                                                        block = primer.getBlockState(var42, var50 + modifier, var45);
                                                    }
                                                } else if (blockBelow == testBlock && this.rand.nextInt(200) == 0) {
                                                    primer.setBlockState(var42, var50, var45, testBlock);
                                                }
                                            }
                                        }
                                    }
