...
import keqing.gtqtspace.api.world.ChunkProviderBase;
import keqing.gtqtspace.api.world.generator.GradientNoise;
import keqing.gtqtspace.api.world.generator.NoiseModule;
import keqing.gtqtspace.api.worldgen.SpecialAsteroidBlock;
import keqing.gtqtspace.api.worldgen.SpecialAsteroidBlockHandler;
import keqing.gtqtspace.api.worldgen.vector.BlockVec3;
import keqing.gtqtspace.world.dims.AsteroidsProvider;
import keqing.gtqtspace.world.worldgen.BiomeAdaptive;
import keqing.gtqtspace.world.worldgen.mapGen.Billowed;
import keqing.gtqtspace.world.worldgen.mapGen.Gradient;
...
import static keqing.gtqtspace.world.biome.GTQTSBiomeHandler.ASTEROIDS_BIOME;
...
public class ChunkProviderAsteroids extends ChunkProviderBase
{
    ...
    final byte ASTEROID_STONE_META_1 = 0;
    final byte ASTEROID_STONE_META_2 = 0;
    ...
    public ChunkProviderAsteroids(World par1World, long par2, boolean par4)
    {
        ...
        this.coreHandler.addBlock(new SpecialAsteroidBlock(Blocks.STONE, (byte) 12, 2, .13));
        ...
        this.shellHandler.addBlock(new SpecialAsteroidBlock(ICE, (byte) 0, 1, .15));
    }
    ...
    private void generateAsteroid(Random rand, int asteroidX, int asteroidY, int asteroidZ, int chunkX, int chunkZ, int size, ChunkPrimer primer, boolean flagDataOnly)
    {
        ...
        {
            isHollow = true;
            shell = new SpecialAsteroidBlock(ICE, (byte) 0, 1, .15);
        }

        // Add to the list of asteroids for external use
        ((AsteroidsProvider) this.world.provider).addAsteroid(asteroidX, asteroidY, asteroidZ, size, isHollow ? -1 : core.index);
        ...
        if (!this.largeAsteroids.isEmpty())
        {
            for (AsteroidData asteroidIndex : new ArrayList<AsteroidData>(this.largeAsteroids))
            {
                ...
                if (rand.nextInt(ChunkProviderAsteroids.FLOWER_CHANCE) == 0)
                {
                    ...
                    int[] types = new int[]
                            {2, 4, 5, 7};
                    new WorldGenFlowers(this.FLOWER, BlockFlower.EnumFlowerType.getType(BlockFlower.EnumFlowerColor.RED, types[rand.nextInt(types.length)])).generate(world, rand,
...
//        boolean flagXZDChunk = w.getChunkProvider().chunkExists(cx - 1, cz - 1);
        ...
        try {
            Field heightMapMinimumField = Chunk.class.getDeclaredField("heightMapMinimum");
            heightMapMinimumField.setAccessible(true);
            heightMapMinimumField.setInt(chunk, Integer.MAX_VALUE);
            ...
            while (k < 16)
            {
                ...
                while (true)
                {
                    if (y > 0)
                    {
                        ...
                        try {
                            Field heightMapMinimumField = ChunkPrimer.class.getDeclaredField("heightMapMinimum");
                            heightMapMinimumField.setAccessible(true);
                            int currentHeightMapMinimum = heightMapMinimumField.getInt(chunk);
                            if (y < currentHeightMapMinimum) {
                                heightMapMinimumField.setInt(chunk,y);
                            }
                        } catch (NoSuchFieldException | IllegalAccessException e) {
                            e.printStackTrace();
                        }
                ...
                for (int z = a.zMinArray; z < a.zMax; z++)
                {
                    for (int y = yMin; y < yMax; y++)
                    {
                        if (chunk.getBlockState(x - 1, y, z).getBlock() instanceof BlockAir && !(chunk.getBlockState(x, y, z).getBlock() instanceof BlockAir))
                        {
                            ...
                            if (count > 12)
                                count = 12;
                            chunk.setBlockState(new BlockPos(x - 1, y, z), Blocks.STONE.getDefaultState());
