package keqing.gtqtspace.api.recipes.machine;
...
public class RecipeMapSF<R extends RecipeBuilder<R>> extends RecipeMap<R> {
    public RecipeMapSF(@Nonnull String unlocalizedName, int maxInputs, int maxOutputs, int maxFluidInputs, int maxFluidOutputs, @Nonnull R defaultRecipeBuilder, boolean isHidden) {
    ...
    protected void addInventorySlotGroup(ModularUI.Builder builder, IItemHandlerModifiable itemHandler, FluidTankList fluidHandler, boolean isOutputs, int yOffset) {
        ...
        if (!isOutputs) {
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    int slotIndex = i * 4 + j;
                    addSlot(builder, startFluidX + 18 * j, startInputsY  + 18 * i, slotIndex, itemHandler, fluidHandler, true, true);
                }
            }
        }
    }
