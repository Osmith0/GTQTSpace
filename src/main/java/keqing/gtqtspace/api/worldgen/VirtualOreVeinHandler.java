...
public class VirtualOreVeinHandler {
    ...
    public static VirtualOreVeinHandler.VirtualOreVeinWorldEntry getVirtualOreVeinWorldEntry( World world, int chunkX, int chunkZ) {
        if (world.isRemote)
            return null;
        ...
        VirtualOreVeinWorldEntry worldEntry = veinCache.get(coords);
        if (worldEntry == null) {
            ...
            worldEntry = new VirtualOreVeinWorldEntry(definition, maximumYields, maximumYields);
    ...
    public static int getTotalWeight( WorldProvider provider, Biome biome) {
        ...
        if (totalWeight == 0 && !virtualOreVeinList.isEmpty())
            GTLog.logger.error("Virtual Ore Vein weight was 0 in biome {}", biome.getBiomeName());
    ...
    public static void recalculateChances(boolean mutePackets) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER && !mutePackets) {
            HashMap<VirtualOreVeinWorldEntry, Integer> packetMap = new HashMap<>();
            for (Map.Entry<ChunkPosDimension, VirtualOreVeinWorldEntry> entry : VirtualOreVeinHandler.veinCache
                    .entrySet()) {
    ...
    public static ObjectList<Integer> getYield(World world, int chunkX, int chunkZ) {
        VirtualOreVeinWorldEntry info = getVirtualOreVeinWorldEntry(world, chunkX, chunkZ);
    ...
    public static class VirtualOreVeinWorldEntry {
        ...
        public static VirtualOreVeinHandler.VirtualOreVeinWorldEntry readFromNBT( NBTTagCompound tag) {
            VirtualOreVeinWorldEntry info = new VirtualOreVeinWorldEntry();
